...
async function registerPairingCode(apiUrl, pairingCode, publicKey, projectName, workingDir, computerName, aiTool, aiToolVersion) {
  ...
  const data = {
    ...
    aiTool,
    aiToolVersion,
    // The pty is started at this size, and the CLI ignores its own SIGWINCH
    // while a phone is attached. Telling the relay lets the phone draw this
    // grid instead of resizing the terminal the user is also working in.
...
async function startCommand(directory, options) {
  try {
    ...
    // Step 7: Register pairing code
    const registration = await registerPairingCode(
      apiUrl,
      pairingCode,
      publicKey,
      projectName,
      workingDir,
      require('os').hostname(),
      // Report a key the server/mobile app already understands. A tool defined
      // in ~/.termly/tools.json can set `protocolKey` to map onto a known tool
      // (e.g. an Ollama model variant reports as "ollama") so mobile rendering
      // and server-side validation keep working for custom entries.
      selectedTool.protocolKey || selectedTool.key,
    ...
    // The relay mints the session id at registration, so adopt it now rather
    // than waiting for pairing_complete. A socket that drops before pairing
    // reconnects with `?sessionId=`, and until this was set that id was a
    // locally generated uuid the relay had never seen - leaving it to guess
    // which session the caller meant.
    if (registration && registration.sessionId) {
    ...
    // Step 10: Create circular buffer
