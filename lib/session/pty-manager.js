...
class PTYManager {
  constructor(tool, workingDir, buffer) {
    ...
    // TUI tools use an alternate screen buffer (internal scroll, mouse events).
    // Declared per tool via the `tui` field, so a tool defined in
    // ~/.termly/tools.json can opt in without touching this file.
    this.isTUIMode = tool.tui === true;
  ...
  start(additionalArgs = []) {
    ...
    try {
      ...
      if (isWindows) {
        // Use cmd.exe to spawn commands on Windows
      ...
      // After reboot, cmd.exe gets system PATH which may not include npm
      // Always prepend npm bin - cmd.exe may not inherit PowerShell's PATH
      ...
      if (isWindows) {
        ...
        spawnEnv.PATH = `${npmGlobalBin};${currentPath}`;
        logger.debug(`Added npm global bin to PATH: ${npmGlobalBin}`);
      }

      // Per-tool environment from the registry / ~/.termly/tools.json.
      // This is how a tool definition selects a model or endpoint, e.g.
      // { "env": { "ANTHROPIC_MODEL": "claude-opus-5" } }
      if (this.tool.env) {
        Object.assign(spawnEnv, this.tool.env);
        logger.debug(`Tool env overrides: ${Object.keys(this.tool.env).join(', ')}`);
      }
  ...
  handlePTYOutput(data) {
    ...
    if (filtered.length > 0) {
      ...
      // Skip buffer for TUI apps - catchup is useless, they redraw full screen
      if (!this.isTUIMode) {
  ...
  // Check if running a TUI app (OpenCode, etc.)
  isTUI() {
